...
HarbourGuard computes an aggregated risk score (0 to 100) per scan. In practice, 8 of 9 images score 100/100. Only alpine:3.19 (6 total findings, 0 CRITICAL) scores below maximum (49/100). The score provides no granularity across the vulnerability spectrum and cannot be used as a policy threshold.
...
### P1 to P3: Scanner-native policies
...
| EPSS (FIRST.org) | `api.first.org/data/v1/epss` | Exploitation probability score (0 to 1, daily) | Risk prioritisation |
...
P1 to P3 operate entirely on scanner output and answer the question: *is this finding present and consistent across tools?* They cannot distinguish between a CVE that is filed-but-unvalidated and one that is confirmed-and-actively-exploited.
...
- **Technically concrete** with working Rego policies, OPA unit tests, and a CI/CD pipeline stage
- **Progressive**: four policies of increasing precision enabling quantitative comparison
- **Generalisable** and not tied to any specific orchestration platform
...
A foundational assumption underlying P1 to P3 is that the correct response to a confirmed vulnerability is to block the build. This is appropriate for findings that are confirmed real, fixable, and actively exploited. It is not appropriate for a substantial portion of what scanners report.
